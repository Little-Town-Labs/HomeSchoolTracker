...
const styles = StyleSheet.create({
  ...
  // New style for subscription required message
...
interface TranscriptPDFProps extends Omit<DocumentProps, "children"> {
  student: Student;
  includeGPA?: boolean;
}
...
export function TranscriptPDF({
  ...
  if (!isAllowed) {
    // Show a message if not subscribed/trialing
    return (
      <Document>
        <Page size="LETTER" style={styles.page}>
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={styles.subscriptionRequired}>
              Subscription Required to Generate Transcript
            </Text>
  ...
  const coursesByGradeLevel = student.courses.reduce(
    (acc, course) => {
  ...
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.studentInfo}>
          ...
          <View style={styles.infoRow}>
            <Text style={styles.label}>
              Student ID: <Text style={styles.value}>{student.info.id}</Text>
            </Text>
            <Text style={styles.label}>
              Date of Birth:{" "}
              <Text style={styles.value}>{student.info.birthDate}</Text>
            </Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.label}>
              Expected Graduation:{" "}
              <Text style={styles.value}>{student.info.graduationDate}</Text>
            </Text>
            {includeGPA && (
              <Text style={styles.label}>
                Cumulative GPA:{" "}
                <Text style={styles.value}>{calculateGPA()}</Text>
              </Text>
            )}
          </View>
        </View>
        ...
        {[9, 10, 11, 12].map(
          (gradeLevel) =>
            coursesByGradeLevel[gradeLevel] && (
              <View key={gradeLevel}>
                <Text
                  style={styles.sectionTitle}
                >{`Grade ${gradeLevel} Courses`}</Text>
        ...
        {/* Test Scores - Only show if there are scores and includeTestScores is true */}
          ...
          <View style={styles.infoRow}>
            <Text style={styles.label}>
              ...
              <Text style={styles.value}>
                {student.transcriptMeta.issueDate}
              </Text>
            </Text>
          </View>
          <View style={styles.signature}>
            <Text style={styles.value}>
              {student.transcriptMeta.administrator}
            </Text>
